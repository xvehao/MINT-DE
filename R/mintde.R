...
#' Apply DeSeq2
#' @description
#' Apply DeSeq2 to different timepoints against baseline.
#'
#'
#' @param dat a matrix of LFC, row for the object, column for the timepoint.
#' @param hours an array of timepoints
#'
#' @return list
#' @export
#'
...
#'
#' @param param parameter associated with each method. For Edgington (sum of p-values) and Fisher (sum of log p-values), an array of topk genes to choose; For commonDE (only thresholding p1 and p2), an array of threshold of p1 and p2
#' @param method string of the method to use, "E" for Edgington (sum of p-values), "F" for Fisher (sum of log p-values), and "V" for commonDE/Venn diagram method (only thresholding p1 and p2)
#' @param p1 array of p-values for the strength of DE in the first modality
#' @param p2 array of p-values for the strength of DE in the second modality
#' @param p3 array of p-values for correlation over time across modalities
#' @param Genes array of gene names
#'
#' @return an array of selected genes
#' @export
#'
...
#' commonDE/Venn Diagram Method
#'
#' @description
#' commonDE/Venn diagram method (only thresholding p-values from two modalities)
#'
#' @param cutoff an array of cutoff (=10^(cutoff)) of p-values
#' @param p1 array of p-values for the strength of DE in the first modality
#' @param p2 array of p-values for the strength of DE in the second modality
#' @param Genes array of gene names
#'
...
#' @export
#'
#' @examples Venn_method(cutoff=c(-2,-3),
#' p1=runif(1000,min=0,max=0.05),
#' p2=runif(1000,min=0,max=0.05),
...
#' Gene Selection based on Edgington's Method
#' @description
#' The core function that select genes differentially expressed (DE) and correlated across modalitiesc based on givens p-values using Edgington's Method (sum of p-values).
#'
#' @param topk an array of topk genes to choose
#' @param p1 array of p-values for the strength of DE in the first modality
#' @param p2 array of p-values for the strength of DE in the second modality
#' @param p3 array of p-values for correlation over time across modalities
#' @param Genes array of gene names
#'
#' @return gene_id gives the array of selected genes, idx gives the corresponding index
#' @export
#'
#' @examples Edgington_method(topk=c(10,50),
#' p1=runif(1000,min=0,max=0.05),
#' p2=runif(1000,min=0,max=0.05),
#' p3=runif(1000,min=0,max=0.05),
#' Genes=paste0('gene',1:1000))
...
#' Gene Selection based on Fisher's Method
#' @description
#' The core function that select genes differentially expressed (DE) and correlated across modalitiesc based on givens p-values using Fisher's Method (sum of log p-values).
#'
#' @param topk an array of topk genes to choose
#' @param p1 array of p-values for the strength of DE in the first modality
#' @param p2 array of p-values for the strength of DE in the second modality
...
#' @export
#'
#' @examples Fisher_method(topk=c(10,50),
#' p1=runif(1000,min=0,max=0.05),
#' p2=runif(1000,min=0,max=0.05),
#' p3=runif(1000,min=0,max=0.05),
#' Genes=paste0('gene',1:1000))
